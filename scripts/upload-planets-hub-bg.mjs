#!/usr/bin/env node
/**
 * One-off: convert + upload the 3 Planets-hub background assets to R2.
 * Source dir: assets-to-upload/Planets
 * Target path: Pillar/Hub/Planets/<filename>.webp
 *
 * Reads R2 creds from .env.local. Real WebP encoding via sharp (the casual
 * `sips ... .jpg` -> upload as .webp pattern would put JPEG bytes behind a
 * .webp filename, breaking WebP-conditional serving and Content-Type).
 */
...
const SRC_DIR = path.join(ROOT, 'assets-to-upload', 'Planets');
const OUT_DIR = path.join(ROOT, 'assets-to-upload', '.optimized', 'Planets');
const R2_PREFIX = 'Pillar/Hub/Planets';

const PLAN = [
  {
    role: 'hero',
    src: 'hero-banner-planets-page.png',
    out: 'hero-banner-planets-page.webp',
    maxBytes: 400 * 1024,
    maxWidth: 1600,
    maxHeight: null,
  },
  {
    role: 'card-bg',
    src: 'planet-card-bg.png',
    out: 'planet-card-bg.webp',
    maxBytes: 300 * 1024,
    maxWidth: 1200,
    maxHeight: null,
  },
  {
    role: 'main-bg',
    src: 'planet-main-page-bg.png',
    out: 'planet-main-page-bg.webp',
    maxBytes: 400 * 1024,
    maxWidth: 1920,
    maxHeight: null,
  },
...
async function main() {
  ...
  for (const item of PLAN) {
    ...
    console.log(
      `  ${r.fits ? 'ok ' : 'WARN'} ${item.out.padEnd(36)} ${fmtBytes(origSize).padStart(10)} -> ${fmtBytes(r.size).padStart(10)} q=${r.quality} ${meta.width}x${meta.height} (-${reduction}%)`,
    );
    results.push({ ...item, srcPath, dstPath, optSize: r.size });
  }
