#!/usr/bin/env node
/**
 * One-off: convert + upload sun-strip-bg.png to R2 as a real WebP.
 * Source: assets-to-upload/Planets/Sun/sun-strip-bg.png
 * Target: Pillar/Planets/Sun/sun-strip-bg.webp
 *
 * Real WebP via sharp (the casual `sips ... .jpg` -> upload as .webp pattern
 * would put JPEG bytes behind a .webp filename, breaking Content-Type).
 */
...
const SRC = path.join(ROOT, 'assets-to-upload', 'Planets', 'Sun', 'sun-strip-bg.png');
const OUT_DIR = path.join(ROOT, 'assets-to-upload', '.optimized', 'Planets', 'Sun');
const OUT = path.join(OUT_DIR, 'sun-strip-bg.webp');
const R2_KEY = 'Pillar/Planets/Sun/sun-strip-bg.webp';
