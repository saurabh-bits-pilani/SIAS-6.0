#!/usr/bin/env node
/**
 * One-off: convert + upload 3 zodiac hub preview assets to R2.
 * Target path: /Zodiac/Hub/preview/<filename>.webp
 * Size budgets: hero <= 300 KB, cards <= 150 KB.
 *
 * Re-encodes at decreasing quality until it fits the budget.
...
const SRC_DIR = path.join(ROOT, 'assets-to-upload');
const OUT_DIR = path.join(SRC_DIR, '.optimized');
const R2_PREFIX = 'Zodiac/Hub/preview';

// Plan: actual on-disk filenames (with dots, not underscores) -> URL-clean
// .webp output names. Spelling preserved per user request ("tarus", "zodia").
const PLAN = [
  {
    role: 'card',
    src: 'aries-card-4.3-ratio-square.png',
    out: 'aries-card-4-3-ratio-square.webp',
    maxBytes: 150 * 1024,
    maxWidth: 800,
    maxHeight: 800,
  },
  {
    role: 'card-circular',
    src: 'tarus-card-1.1_ratio-circular.png',
    out: 'tarus-card-1-1-ratio-circular.webp',
    maxBytes: 150 * 1024,
    maxWidth: 800,
    maxHeight: 800,
    preserveAlpha: true, // circular crop -> alpha edges
  },
  {
    role: 'hero',
    src: 'zodia-hero-banner-1.png',
    out: 'zodia-hero-banner-1.webp',
    maxBytes: 300 * 1024,
    maxWidth: 1600,
    maxHeight: null, // preserve aspect
  },
];
...
async function encodeToFitBudget(srcPath, dstPath, item) {
  // Try descending quality until under budget. Start optimistic.
...
async function main() {
  ...
  console.log('\n=== Phase 1: Convert PNG -> WebP (budget: card <= 150 KB, hero <= 300 KB) ===');
  for (const item of PLAN) {
    ...
    if (!r.fits) {
      console.log(`       budget ${fmtBytes(item.maxBytes)} exceeded at q=${r.quality}; will upload anyway`);
    }
  ...
  for (const r of results) {
    ...
    console.log(`  ${res.ok ? 'ok ' : 'FAIL '} HTTP ${res.status} ${r.publicCT}  ${r.url}`);
  }
